...
func _on_cat_avatar_pressed() -> void:
	...
	if lvl == 0 or lvl == 1 and GameManager and GameManager.current_level_index == 0:
		...
		hint_text = "(=^･ω･^=) В Хиве у спавна взломайте матрицу 1-9 быстрой реакцией (клавиши 1-9 или клик за 2.0с). Затем DAU расчищает завал на восточный склад за батареей и ставит тяжелый ящик на нажимную плиту в ангаре, чтобы запитать терминал выхода для JAM!"
	elif lvl == 3 or lvl == 4:
